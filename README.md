...
- 顶栏收敛为「陪玩 / 商城 / 我的」，右侧为心愿单 / 购物车 / 点单车统一侧栏入口；主题、语言、退出收入头像菜单
- 侧栏打开时锁定页面滚动（消除外层滚动条），列表区内滚动，底栏固定展示全选 / 合计 / 结算
...
- 清单页暗色会所皮肤：Hero、标签筛选、竖版三列名片；hover 露出「查看名片 / 立即点单」
- 列表加载与商城一致：三列 4:3 骨架占位 + 扫光；失败 / 空列表仍用居中空态
- 「今晚推荐」取当前页前 3 条做预览编排（标「预览」），非独立推荐接口；无价格 / 在线排序 / 评分
- 公开名片：身份栏 + 相册 + 点单面板（快捷日期 / 半小时时段 / 自定义时间 / 1–3 小时 / 点单摘要），支持立即点单与加入点单车；评价区待接口，暂用占位文案
- 收藏仍存浏览器本地；时薪与真实评价未接入，文案如实说明
...
```text
app/
  components/          通用组件（导航、页脚、统一侧栏抽屉、陪玩卡片等）
  composables/         多语言、主题、认证状态、商城心愿单/购物车、点单车与 API 调用
...
- 陪玩详情点单面板升级为三步式预约：日期、快捷或自定义时间、服务时长与跨日结束时间实时联动；增加提交摘要、提前 15 分钟校验，以及「立即点单 / 加入点单车」双入口，完整适配明暗主题。

- 陪玩清单加载态改为与商城一致的骨架占位：三列 4:3 名片位 + 扫光动画，去掉居中「正在加载陪玩」文案块；失败/空列表仍用原空态。
- 陪玩清单 Hero 改为通栏出血：去掉首屏两侧 24px 留白与顶部间距，直接贴住顶栏，内部标题、搜索与筛选仍按内容宽度居中；下方列表继续保持 1200px 内容宽。
- 补齐浅色主题下「今晚推荐」侧卡的配色：无封面媒体块由近黑改为淡紫底、姓氏标记与收藏按钮改用浅色描边，不再在暖白卡上出现深色方块。
- 修复浅色主题下陪玩 Hero 主标题与搜索文字被全局浅色变量改成深色、落在夜景背景上几乎不可见的问题；夜景区域固定使用高对比浅色文字。
- 陪玩清单与详情补齐双主题：深色保留会所夜景，浅色切换为暖白紫金页面与信息面板，并单独校正标签、输入框、状态和媒体卡的文字对比度。
- 陪玩详情提高设计稿还原度：扩大主体宽度，左栏加入封面式身份档案，中栏升级为带渐变标题和切换控件的主相册，右栏强化状态与点单层级，下方信息卡同步收紧。
- 「全部陪玩」名片由过高的竖海报收紧为设计稿的 4:3 三列图卡，昵称、简介、标签与入口叠放在图片底部，移除会撑高卡片的 hover 双按钮。
- 陪玩清单 Hero 按设计稿重构为居中场景式构图：星夜会所背景、居中标题与状态、圆角搜索框、右侧箭头及图标筛选条统一收进首屏视觉。
- 修复会所版陪玩清单仍受旧横向名录 CSS 影响的问题：推荐主卡改为整图横幅叠字，普通名片稳定为三列竖卡。
- 陪玩清单 / 公开名片第一期会所视觉落地：暗色竖卡三列、「今晚推荐」预览编排、详情三栏点单；不伪造价格与评价，样式见 `companion-club.css`。
- 陪玩清单「公会陪玩」改为横向名录卡（左媒体右信息、双列排布），去掉竖版大海报与封面玻璃态状态条，无图用克制圆标。
- 陪玩清单「公会陪玩」卡片收紧：封面降至约 168px，无图改为中号圆标，去掉正文重复小头像，压缩标题/简介/底栏间距。
- 顶栏心愿单图标补回悬停 `title`（「心愿单」），与购物车 / 点单车一致。
- 修复侧栏 hover 发糊：遮罩 `backdrop-filter` 与半透明面板叠层导致文字发虚；模糊改到遮罩伪元素，面板改不透明，移除按钮禁用 transform。
- 侧栏「移除」改为右侧玫红描边胶囊按钮（文案可见），不再使用不易发现的灰色 ×。
- 顶栏侧栏（购物车 / 心愿单 / 点单车）视觉精修：锁住页面滚动消除外层滚动条，标题收紧，商品行改为横排缩略字标 + 自定义勾选，全选与合计沉底；结算文案改为「去结算」。
- 点单车改为顶栏日历图标 + 统一侧栏抽屉（与心愿单 / 购物车共用 `SideDrawer`）；`/bookings/cart` 深链回跳打开侧栏。
- 顶栏购物车图标改绑商品购物车（全站抽屉 + 角标），点单车移入头像菜单；商城页与顶栏共用 `useCommerceCart` 状态。
