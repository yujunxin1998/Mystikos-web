...
const companionCopy = computed(() => locale.value === 'zh' ? {
  eyebrow: '陪玩身份', title: '把熟悉的游戏，分享给更多同行者。', body: '所有用户默认都是 Member。提交申请并通过线下考核后，你会额外获得 Companion 身份。', apply: '申请成为陪玩', view: '查看申请进度', security: '账号与联系方式',
  status: { PENDING: '申请中', ASSESSING: '考核中', APPROVED: '审核通过', REJECTED: '审核未通过' }
} : {
  eyebrow: 'Companion identity', title: 'Share the games you know with someone new.', body: 'Everyone begins as a Member. After applying and completing the offline assessment, Companion is added to your account.', apply: 'Become a companion', view: 'View application', security: 'Account & contact',
    ...
    <section class="profile-section companion-identity-card">
      <div class="companion-identity-copy">
        ...
        <div class="companion-identity-actions">
          <NuxtLink :to="companionApplication ? '/companion/application' : '/companion/apply'" class="button button-primary">{{ companionApplication ? companionCopy.view : companionCopy.apply }} <span>→</span></NuxtLink>
          <NuxtLink to="/account-security" class="button button-ghost">{{ companionCopy.security }}</NuxtLink>
        </div>
      </div>
