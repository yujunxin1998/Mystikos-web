...
const copy = computed(() => locale.value === 'zh' ? {
  ...
  empty: 'No companion application yet', emptyBody: 'When you are ready, share your games and preferred contact.', apply: 'Start application', submitted: 'Submitted', updated: 'Last updated', contact: 'Contact', games: 'Games', applicant: 'Applicant', gameNickname: 'Game nickname', region: 'Region', intro: 'Introduction', assessor: 'Assessor', result: 'Assessment result', opinion: 'Review note', pendingResult: 'The result will appear here after the offline assessment.',
...
const maskedContact = computed(() => {
  ...
  return application.value?.contactType === 'EMAIL' ? maskEmail(value) : maskPhone(value)
})
    ...
    <template v-else>
      ...
      <section class="application-dossier">
        <div class="dossier-main">
          ...
          <dl class="dossier-meta"><div><dt>{{ copy.gameNickname }}</dt><dd>{{ application.gameNickname || '—' }}</dd></div><div><dt>{{ copy.region }}</dt><dd>{{ application.regionCode || '—' }}</dd></div><div><dt>{{ copy.contact }}</dt><dd>{{ maskedContact }}</dd></div><div><dt>{{ copy.submitted }}</dt><dd>{{ formatDate(application.submittedAt) }}</dd></div><div><dt>{{ copy.updated }}</dt><dd>{{ formatDate(application.updatedAt) }}</dd></div></dl>
