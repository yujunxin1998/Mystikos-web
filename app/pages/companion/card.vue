...
const form = reactive({ displayName: '', tagline: '', bio: '', tagIds: [] as number[], availability: '今晚可约', coverName: '', coverDataUrl: '', coverObjectKey: '', photos: [] as CardPhoto[], videos: [] as CardVideo[], audios: [] as CardAudio[] })
    ...
    <template v-else>
      ...
      <main class="card-editor-workspace">
        ...
        <div class="card-form-column" :class="{ 'is-editing-locked': editingLocked }" :inert="editingLocked"><p class="card-panel-label"><span>{{ copy.edit }}</span><span>{{ form.bio.length }}/300</span></p>
          <section class="card-edit-section"><header><i>01</i><div><h2>{{ copy.basic }}</h2><p>展示名称、标签与可预约状态</p></div></header><div class="card-fields"><label><span>展示名称</span><input v-model.trim="form.displayName" maxlength="24"></label><label><span>一句话标签</span><input v-model.trim="form.tagline" maxlength="42"></label><label><span>在线状态</span><select v-model="form.availability"><option>今晚可约</option><option>周末可约</option><option>暂不接单</option></select></label><fieldset><legend>{{ copy.games }} <small>{{ copy.max }}</small></legend><div class="card-tag-picker"><button v-for="tag in availableTags" :key="tag.id" type="button" :class="{ selected: form.tagIds.includes(tag.id) }" @click="toggleTag(tag.id)">{{ tag.label }}</button></div></fieldset></div></section>
