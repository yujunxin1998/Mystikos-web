...
const form = reactive({ applicantName: '', regionCode: '', introduction: '', tagIds: [] as number[], contactType: 'PHONE' as 'PHONE' | 'EMAIL', contactValue: '' })
...
const copy = computed(() => locale.value === 'zh' ? {
  ...
  basics: '申请人资料', name: '申请人姓名或称呼', namePlaceholder: '管理员联系你时使用的称呼', region: '所在国家或地区', intro: '自我介绍', introPlaceholder: '介绍你的游戏经验、沟通方式和擅长营造的游戏氛围…',
  ...
  basics: 'Applicant details', name: 'Name or preferred name', namePlaceholder: 'How administrators should address you', region: 'Country or region', intro: 'About your play style', introPlaceholder: 'Describe your experience, communication style, and the kind of game atmosphere you create…',
...
const submit = async () => {
  error.value = ''
  if (!form.applicantName.trim() || !form.regionCode || form.introduction.trim().length < 40 || !form.tagIds.length || !accepted.value) { error.value = copy.value.required; return }
  ...
  saving.value = true
  try {
    const selectedTags = tags.value.filter(tag => form.tagIds.includes(tag.id)).map(tag => ({ id: tag.id, label: tag.label }))
    await companionApi.submitApplication({ ...form, applicantName: form.applicantName.trim(), introduction: form.introduction.trim(), contactValue, phoneCountry: phoneCountry.value }, selectedTags)
...
onMounted(async () => {
  ...
  try {
    ...
    const readiness = await companionApi.loadAccountCompletion(currentProfile)
...
<template>
  <div class="companion-page section-wrap">
    ...
    <section v-else-if="!companionApi.accountCompletion.value?.companionApplicationAllowed" class="application-blocked">
      <span>!</span><div><h2>{{ copy.blockedTitle }}</h2><p>{{ copy.blockedBody }}</p></div><NuxtLink to="/account-security" class="button button-primary">{{ copy.complete }} →</NuxtLink>
    </section>
    ...
    <form v-else class="application-form" @submit.prevent="submit">
      <section class="application-section">
        ...
        <div class="companion-fields">
          ...
          <label class="companion-field companion-field-wide"><span>{{ copy.intro }}</span><textarea v-model="form.introduction" rows="6" minlength="40" maxlength="800" :placeholder="copy.introPlaceholder" /><small>{{ form.introduction.length }} / 800 · min 40</small></label>
        </div>
      </section>
